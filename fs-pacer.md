...
The early Pacer was written in C, using the infamously complicated [libck](http://concurrencykit.org/) for lock-free data structures. Lock-free data structures and algorithms, ensure that at least one thread can keep going. They are fun to think about and implement from scratch.

That being said, the architecture of the old code makes it highly susceptible to race conditions and spin-locks even when using the [lock-free data structures](http://www.drdobbs.com/lock-free-data-structures/184401865) (Lock-free data structures rely on [atomic](https://software.intel.com/en-us/node/506090) operations for their functionalities). On a bad day, some Ad campaign to misbehave on external state updates, making the use of libck, literally, a ticking atomic counter bomb...

I have hardly seen a real excuse to use lock-free data structures, unless you are doing very low level [kernel](https://lwn.net/Articles/262464/) stuff, a concurrent renderer (Most of I have seen are over engineered and didn't really need it) or your own custom [VM](https://github.com/eloraiby/tcpm) infrastructure (Read that warning twice). Both Erlang & Go don't use them, and both are highly concurrent.

Lock-free data structures and programs are tricky and very hard to debug. To get them right you often need formal proofs or tons of tests on different CPU architectures. If not, brace yourself for bugs when you try to run your program on a non x64 architecture or change compiler versions (I always test on ARM for their weaker memory model - and so should you).
...
The original pacer would accept a batch of bid requests, run the strategies and reply in less than 2ms. It could answer up to a 1M bid req/s. However, all this was done on a single core. While budget changes and configuration reload (that could happen every 5 minutes) was run in parallel.
